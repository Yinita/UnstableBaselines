...
WANDB_RUN_NAME = f"Batch-5-Experiment-0--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"
...
try:
    collector.collect.remote(num_workers=130, num_eval_workers=16)
