...
from torch.distributed.fsdp import StateDictType, FullStateDictConfig, BackwardPrefetch
...
def no_lora_policy(module):
    return not any("lora" in name.lower() for name, _ in module.named_modules())
...
class Learner:
    def __init__(
        ...
        if self.activation_checkpointing:
            # apply_activation_checkpointing(self.model, checkpoint_wrapper_fn=checkpoint_wrapper, check_fn=no_lora_policy)
            # apply_activation_checkpointing(self.model, checkpoint_wrapper_fn=checkpoint_wrapper)
            apply_activation_checkpointing(self.model, checkpoint_wrapper_fn=checkpoint_wrapper, check_fn=skip_lora)
    ...
    def _save_checkpoint(self):
        ...
        if dist.get_rank() == 0:
            ...
            lora_sd_fixed = {}
            for k, v in lora_sd_raw.items():
                k = k.replace("base_model.model.", "", 1)     # old fix
                k = k.replace(".default", "")                 # old fix
                k = k.replace("._checkpoint_wrapped_module", "")   # NEW: strip wrappers
                lora_sd_fixed[k] = v
