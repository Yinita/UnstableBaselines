...
def _system_panel(sys_stats: Dict[str, Any]) -> Panel:
    text = Text()
    text.append(f"CPU %    : {sys_stats['cpu']:.1f}\n")
    text.append(f"RAM used : {sys_stats['ram']:.1f} GB\n")
    # loop over each GPU
    for gpu in sys_stats.get("gpus", []):
        ...
        power = gpu["power"]
        mem   = gpu["mem"]
        text.append(f"GPU{idx} mem : {mem:.1f} GB\n")
        text.append(f"GPU{idx} power: {power:.0f} W\n")
    text.append(f"buffer sz: {sys_stats['buffer']}\n")
    text.append(f"updated  : {datetime.now().strftime('%H:%M:%S')}")
    return Panel(text, title="System", box=box.ROUNDED)
...
class TerminalInterface:
    ...
    async def _system_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(count):
                ...
                power = pynvml.nvmlDeviceGetPowerUsage(h) / 1000.0  # Watts
                mem   = pynvml.nvmlDeviceGetMemoryInfo(h).used / 1e9  # GB
                gpus.append({"id": i, "power": power, "mem": mem})
        except Exception:
            pass

        return {"cpu": cpu, "ram": ram, "buffer": buf, "gpus": gpus}
    ...
    async def run(self):
        ...
        # Then split each band into two side-by-side panels
