...
class VLLMActor:
    def __init__(self, vllm_config: Dict[str, Any], tracker, name: str):
        ...
        self._q: deque[tuple[str, Optional[str], asyncio.Future]] = deque()
    ...
    async def submit_prompt(self, prompt: str, lora_path: Optional[str] = None) -> str:
        fut = asyncio.Future()
        self.queued += 1
        self._q.append((prompt, lora_path, fut))
        return await fut

    async def _batch_loop(self):
        while True:
            while self._q:
                prompt, lora_path, fut = self._q.popleft()
                req_id = str(self._next_id); self._next_id += 1
                self._futs[req_id] = fut

                lora_req = None
                if lora_path:
                    lora_req = LoRARequest(lora_path, 1, lora_path)

                self.engine.add_request(req_id, prompt, self.sampling, lora_request=lora_req)
                self.queued -= 1; self.running += 1
            ...
            for out in outs:
                ...
                if seg.finish_reason is not None:
                    ...
                    self.running -= 1
                now = time.monotonic()
                for _ in range(new_tok): self._tok_hist[lora].append(now)
    ...
    async def _report_loop(self):
        while True:
            ...
            try: await self.tracker.log_inference.remote(actor=self.name, gpu_ids=self.gpu_ids, stats=stats)
