...
def _scaled_hist(hist: deque[float|int], width: int) -> str:
    ...
    buckets = [list(hist)[i:i+step] for i in range(0, len(hist), step)]
    result = ""
    for bucket in buckets[:width]:
        if not bucket: result += " "; continue
        avg = sum(bucket) / len(bucket)
        level = int((avg - min(hist)) / (max(hist) - min(hist) if max(hist) != min(hist) else 1e-5) * (len(_HIST_BARS) - 1))
...
class TerminalInterface:
    ...
    def _gpu(self) -> Panel:
        ...
        for gpu_d in self._gpu_stats:
            tok_s = self._tracker_stats["gpu_tok_s"].get(gpu_d["id"], 0)
    ...
    def _ts(self) -> Panel:
        ...
        max_mu = max(mu for _, mu, _ in entries)
        bar_lines = []
    ...
    def _heatmap(self) -> Panel: 
        ...
        tbl.add_row("")  # top-left blank corner
        header = [""] + [Text(f"{self._ts_idx[u]:>2}", style="bold") for u in uid_by_row]
        tbl.add_row(*header)
