...
class TerminalInterface:
    ...
    def _gpu_panel(self, sys: Dict[str, Any]) -> Panel:
        # print(sys)
        blocks = [self._gpu_block(g, sys["gpu_tok"].get(g["id"], 0.0)) for g in sys["gpus"]]
    ...
    async def run(self):
        ...
        asyncio.create_task(self._fetch_loop()) # Launch background fetcher
        with Live(layout, refresh_per_second=2.15):  # display update speed
            while True:
                if self._latest_stats:
                    layout["gpu_half"].update(self._gpu_panel(self._latest_stats))
                await asyncio.sleep(0.5)  # light refresh wait
