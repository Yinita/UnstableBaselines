...
# def _inference_panel(stats: Dict[str, Dict[str, float]], tok_graphs: Dict[str, BufferedHistoryGraph], console_width: int) -> Panel:
#     # Main table showing current stats
#     tbl = Table(box=box.SIMPLE, show_header=True, header_style="bold cyan")
#     tbl.add_column("LoRA",           style="bold")
#     tbl.add_column("Queued",  justify="right")
...
#         tok_rate = meta.get('tok_s', 0)
#         # Update the graph for this LoRA
#         if name not in tok_graphs:
#             tok_graphs[name] = BufferedHistoryGraph(
#                 10_000.0,  # Adjust based on expected max token rate
#                 width=console_width // 3 - 10,
#                 height=4,
#                 format=lambda v: f"{v:.0f}",
#                 dynamic_bound=True
#             )
#         tok_graphs[name].add(tok_rate)
...
#         tbl.add_row(
#             name,
#             f"[{queue_style}]{meta.get('queue', 0)}[/]",
#             f"[{running_style}]{meta.get('running', 0)}[/]",
#             f"[{tok_style}]{tok_rate:.0f}[/]"
#         )
    
#     # Create history graphs for active LoRAs
#     graph_panels = []
#     for name, graph in sorted(tok_graphs.items()):
#         if name in stats:  # Only show graphs for currently active LoRAs
#             current_rate = stats[name].get('tok_s', 0)
...
#             # Truncate graph lines to fit panel width
#             graph_str = '\n'.join([line[-(console_width // 3 - 12):] for line in graph.graph])
            
#             title = Text(f"{name} Rate ({current_rate:.1f} tok/s)", style=style)
#             graph_panels.append(Panel(graph_str, title=title, box=box.SQUARE, width=console_width//3-5, style=style))
...
#         self.gpu_graphs: Dict[int, BufferedHistoryGraph] = {}
#         for i in range(self.gpu_count):
#             self.gpu_graphs[i] = BufferedHistoryGraph(
#                 100.0, 
#                 width=self.console.size.width//4-5, 
#                 height=3, 
#                 format=lambda v: f"{v:.0f}%", 
#                 dynamic_bound=False
#             )
        
#         # Token rate graphs for each LoRA
#         self.tok_graphs: Dict[str, BufferedHistoryGraph] = {}
...
#     async def _inference_stats(self):
#         try:                
#             return await self.tracker.get_latest_inference_metrics.remote()
#         except Exception:   
#             return {}
...
#                     total = mem.total / 1e9
#                     gpus.append({
#                         'id': i, 'used': used, 'total': total, 
#                         'power': power, 'limit': limit, 'pct': pct
#                     })
#             except Exception:
#                 pass

#         return {
#             'cpu': cpu, 'ram_used': ram_used, 'ram_pct': ram_pct, 
#             'buffer': buf, 'gpus': gpus
#         }
...
#             self.gpu_graphs[gpu['id']].add(gpu['pct'])
#             graph_str = '\n'.join([
#                 line[-(self.console.size.width // 4 - 9):] 
#                 for line in self.gpu_graphs[gpu['id']].graph
#             ])
#             title = Text(f"GPU {gpu['id']} Power History ({gpu['pct']:.2f}%)", style=style)
#             panels.append(Panel(
#                 graph_str, 
...
#         top = Panel(txt, title='System Overview', box=box.ROUNDED)
#         return Panel(
#             Columns([top, Columns(panels, expand=True)], expand=True), 
#             title='System', 
#             box=box.DOUBLE
#         )
