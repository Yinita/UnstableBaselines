    ...
    def _system_panel(self, sys_stats: Dict[str, Any]) -> Panel:
        ...
        for gpu in sys_stats['gpus']:
            ...
            if idx not in self.gpu_graphs:
                # print("WIDTH", self.console.size.width//4-6)
                self.gpu_graphs[idx]=BufferedHistoryGraph(100.0, width=self.console.size.width//4-5, height=3, format=lambda v:f"{v:.0f}%",dynamic_bound=False)
            
            graph = self.gpu_graphs[idx]
            # print("GPU PCT", gpu['pct'])
            graph.add(gpu['pct'])
            # input(graph.graph)
            body = '\n'.join(graph.graph)
            panels.append(Panel(body, title=f"GPU {idx} Power History ({gpu['pct']:.2f}%)", box=box.SQUARE, width=self.console.size.width//4-5))
