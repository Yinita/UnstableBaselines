...
#             try:
#                 self.logger.debug(f"About to call engine.step(), running requests: {sum(self._running.values())}")
#                 outs = self.engine.step()
#                 self.logger.debug(f"engine.step() returned {len(outs)} outputs")
#             except Exception as exc:
#                 self.logger.exception(f"engine.step() failed - running: {self._running}, queued: {self._queued}")
#                 # Don't just raise - this might cause Ray actor to become unresponsive
#                 # Consider how to handle this more gracefully
#                 raise


#             for out in outs:
#                 self.logger.warning(f"Engine returned no outputs but {sum(self._running.values())} requests running")
#                 req_id = out.request_id
#                 lora = self._req2lora.get(req_id, "base")
#                 segment = out.outputs[-1]
