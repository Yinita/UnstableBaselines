...
# ----------------- dashboard.py -----------------
def _inference_panel(stats: Dict[str, Dict[str, float]]) -> Panel:
    ...
    for name, meta in sorted(stats.items()):
        tbl.add_row(name,
                    str(meta.get("queue", 0)),
                    str(meta.get("running", 0)),
                    f"{meta.get('tok_s', 0):.0f}")
    return Panel(tbl, title="Inference (queues & rates)", box=box.ROUNDED)
...
class TerminalInterface:
    ...
    async def _inference_stats(self):
        try:                return await self.tracker.get_latest_inference_metrics.remote()
    ...
    def _system_panel(self, sys_stats: Dict[str, Any]) -> Panel:
        ...
        for gpu in sys_stats['gpus']:
            ...
            # graph_str = '\n'.join(self.gpu_graphs[gpu['id']].graph)
