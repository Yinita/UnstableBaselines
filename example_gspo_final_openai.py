...
print(f"Initializing GSPO training with OpenAI Agent opponent...")
print(f"  - Training Model: {MODEL_NAME}")
print(f"  - Opponent Model: {OPENAI_MODEL_NAME} (OpenAI)")
print(f"  - OpenAI Base URL: {OPENAI_BASE_URL}")

# initialize environment scheduler with OpenAI agent as fixed opponent
env_sampler = unstable.samplers.env_samplers.UniformRandomEnvSampler(
    ...
    eval_env_specs=[
        # Use our OpenAI agent as the fixed opponent
        unstable.EvalEnvSpec(env_id="SimpleTak-v0-train", num_players=2, prompt_template="qwen3-zs", fixed_opponent=OPENAI_OPPONENT_NAME),
        unstable.EvalEnvSpec(env_id="KuhnPoker-v0-train", num_players=2, prompt_template="qwen3-zs", fixed_opponent=OPENAI_OPPONENT_NAME),
])

# Tracker
tracker = unstable.Tracker.options(name="Tracker").remote(
    run_name=f"GSPO-OpenAI-{MODEL_NAME.split('/')[-1]}-{env_sampler.env_list()}-{int(time.time())}", 
...
print(f"\nStarting GSPO training with OpenAI Agent opponent:")
print(f"  - Training Model: {MODEL_NAME}")
