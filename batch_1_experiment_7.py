...
WANDB_RUN_NAME = f"Batch-1-Experiment-7--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"
...
step_reward_transformation = retra.ComposeStepRewardTransforms([
    retra.RewardForThinkTags(reward=1.0), # +0.25 for using the correct format
    retra.PenaltyForInvalidMove(reward= 0.0, penalty= -1.0), 
])
