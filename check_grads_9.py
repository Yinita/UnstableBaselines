...
parser.add_argument("--seq",   type=int,   default=6000)
...
def run_ckpt_cpu_offload(ids):
    model = make_model()
    
    # Apply full activation checkpointing with CPU offloading
    apply_activation_checkpointing(
        model,
        checkpoint_wrapper_fn=lambda m: checkpoint_wrapper(m, offload_to_cpu=True),
        check_fn=make_checkpoint_filter(1.0)
    )
    model.enable_input_require_grads()
    model.gradient_checkpointing_enable()
    # optimizer = torch.optimizer.Adam
    torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-6)
...
for pct in (1.0,1.0,1.0,): #, 0.8):
    print(f"🟦  CKPT {int(pct*100):>3d}% of blocks")
    ck_mem = run_ckpt(ids, pct)
    torch.cuda.empty_cache()


print(f"\n🟦  CKPT 100% + CPU offload")
ckpt_offload_mem = run_ckpt_cpu_offload(ids)