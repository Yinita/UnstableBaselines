...
def validate_requested_gpus(args):
    ...
    cpu_reserved = args.num_learners * 2
    ...
    def line(label, used, available, symbol=""):
        print(f"{label:<40} {used:>5} / {available:<5} {symbol}")

    line("GPUs for Actors", args.num_actors, available_gpus)
    line("GPUs for Learners", args.num_learners, available_gpus)
    line("Total GPUs Requested", total_gpus, available_gpus,
         "✅" if total_gpus == available_gpus else ("⚠️" if total_gpus < available_gpus else "❌"))
