...
WANDB_RUN_NAME = f"Batch-4-Experiment-24--{MODEL_NAME.split('/')[-1]}-[{','.join([t[0] for t in TRAINING_ENVS])}]-{int(time.time())}"
...
ray.get(model_pool.add_fixed.remote(name="meta-llama/llama-3.3-70b-instruct")) # add fixed opponents
ray.get(model_pool.add_fixed.remote(name="google/gemini-2.0-flash-001")) # add fixed opponents
ray.get(model_pool.add_fixed.remote(name="meta-llama/llama-4-scout")) # add fixed opponents
...
try:
    collector.collect.remote(num_workers=768, num_eval_workers=0)
